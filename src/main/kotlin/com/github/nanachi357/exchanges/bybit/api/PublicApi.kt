...
object PublicApi {
    ...
    fun symbols(): Result<Symbols> {
        return runCatching {
            sendRequest<Symbols>(HttpMethod.Get, SYMBOLS, query = mapOf("category" to "spot"))?.result 
                ?: throw IllegalStateException("Empty response from symbols endpoint")
        }
    }

    fun klines(coin: String): Result<Klines> {
        val query = mapOf(
            "category" to "spot",
            "symbol" to coin + "USDT",
            "interval" to "60",
        ...
        return runCatching {
            sendRequest<Klines>(HttpMethod.Get, KLINES, query = query)?.result
                ?: throw IllegalStateException("Empty response from klines endpoint")
        }
    }

    fun currencies(): Result<Currencies> {
        val headers = ApiUtils.genGetSign(emptyMap())
        return runCatching {
            sendRequest<Currencies>(HttpMethod.Get, CURRENCIES, headers = headers)?.result
                ?: throw IllegalStateException("Empty response from currencies endpoint")
        }
    }

    fun fees(symbol: String): Result<Fees> {
        val query = mapOf("category" to "spot", "symbol" to symbol)
        val headers = ApiUtils.genGetSign(query)
        return runCatching {
            sendRequest<Fees>(HttpMethod.Get, FEES, headers = headers, query = query)?.result
                ?: throw IllegalStateException("Empty response from fees endpoint")
        }
    }
